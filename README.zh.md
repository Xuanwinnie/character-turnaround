...
生成轉面圖時，Skill 會先判斷所有配件與非對稱特徵的實際附著位置、朝向、角色自身左右、前後遮擋與根部連接，再推導各視角的可見性。胸前領巾結、蝴蝶結或徽章只是例子：任何正面專屬元素轉到不可見的一面，都不得直接複製成背面裝飾；尾巴等非對稱特徵也必須依空間關係推導，不以畫面左右任意鏡像。若任一視角違反附著位置、遮擋或旋轉連續性，該視角會被判定為 QA 失敗並重試一次。

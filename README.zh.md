...
它適合角色、吉祥物、品牌 IP、遊戲、動畫、漫畫與繪本創作。核心流程採媒材中立設計，可作為 2D、3D 或兩者共用的角色參考。輸出的 Character Bible 與參考圖可直接交給 `ip-asset-generator`，用於後續生成場景圖、貼圖、社群圖片與透明角色素材。
...
- 由 AI 根據參考圖估算比例鎖定，協助維持角色大小、頭身比與配件位置。
- 逐視角執行 QA，且只重試失敗視角一次。
- 支援可選的 `asset_mode`（`2d`、`3d` 或 `both`），不會要求 2D 流程填寫 3D 資訊。

## 媒材中立流程

在 Character Bible 規劃階段選擇 `asset_mode: 2d|3d|both`；這不會改變四視角的核心流程。

- `2d`（預設）：著重畫風、線條、色彩，以及插畫、貼圖、漫畫與場景素材等下游用途。
- `3d`：可選擇補充可觀察的立體結構、材質與可動部位，供建模交接參考。
- `both`：維持一份共用身份規格，同時提供 2D 與 3D 的可選交接資訊。

本 Skill 不會建立 mesh、拓撲、骨架或動畫；任何 3D 資訊都只是明確標示的參考交接，不能假裝成已確認的技術規格。
...
比例鎖定表由 AI 自動從圖檔估算，使用相對比例與可信度，不要求使用者自行量測。它是柔性約束；如果估算與參考圖或使用者確認衝突，應以參考圖與確認結果為準。
...
生成轉面圖時，Skill 會先判斷所有配件與非對稱特徵的實際附著位置、朝向、角色自身左右、前後遮擋與根部連接，再推導各視角的可見程度。可見性依視角與遮擋幾何判斷，而不是把正面與背面簡化成一律顯示或一律消失；例如胸前領巾結、蝴蝶結或徽章在四分之三正面可能部分可見，但若位於前側且被身體遮擋，背面不得出現。尾巴等非對稱特徵也必須依空間關係推導，不以畫面左右任意鏡像。若任一視角違反附著位置、預期可見程度、遮擋或旋轉連續性，該視角會被判定為 QA 失敗並重試一次。

...
- 對有根部或分段的延伸肢體（尾巴、翅膀、角、辮子等），明確鎖定根部所在身體區域、根部應被身體遮住的程度，以及「由根部到末端」的色段／花紋順序。禁止只描述畫面中的顏色位置，因為角色旋轉後畫面左右會改變。若根部位置或色段順序無法由參考圖確認，必須在生成前請使用者選定，或採保守補完並標記為推測。
...
- 生成前在內部完成一份「視角轉換檢查表」：對每個配件與非對稱特徵記錄附著部位、角色自身左右、朝向、四個視角的預期可見程度（不可見／部分可見／完整可見）、遮擋關係與根部連接。對 `spatial_feature_contract` 內標為 `critical` 的項目，還必須記錄每視角的預期數量、色段順序與 `fail` 條件。這份檢查不必在純圖片模式交付，但必須影響 prompt 與 QA。
...
生成 prompt 順序：身份鎖定與參考圖用途 → 必須保留的 3–8 個錨點與 fixed rules → 先寫出該視角的方位與鏡頭條件 → `spatial_feature_contract` 中該視角的數量、連接方式、角色自身左右、方向、圖樣連續性與遮擋規則 → 中性站姿、畫布、比例、基準線與鏡頭 → visual language、配色、材質、光線 → hard avoids 與未知細節政策。不可只寫「保留某個特徵」；必須把 `critical` 特徵寫成可判定的條件，例如「三分之四正面必須有兩眼，遠側眼部分可見」或「延伸構件從已確認的後側附著點長出，根部被身體遮擋，圖樣順序不可反轉」。對背面視圖要明確寫出「位於前側且被身體遮擋的正面專屬元素不可見」，並說明哪些結構因繞過身體或延伸到背面而可見；不能只寫「正面配件不可見」或「保留配件」。

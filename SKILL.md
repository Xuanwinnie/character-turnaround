...
- 先判斷每個配件的附著位置、朝向與遮擋關係：位於胸前的蝴蝶結、領巾結、徽章或口袋，轉到背面後應被身體遮住；只有確實繞過頸部或延伸到背面的布料／結構才可見。
- 對尾巴、耳朵、斑紋、背帶等非對稱特徵，建立「角色自身左右」而非「畫面左右」的標記。若正面圖中尾巴位於畫面右側，背面圖通常應位於畫面左側；四分之三與側面視角則依旋轉連續變化，不可任意換邊。
...
逐張檢查並記錄：身份、臉型／頭部比例、身體比例、比例鎖定表的相對關係、服裝、招牌配件、主色／輔色、四分之三角度的臉部與配件連續性、角色自身左右是否正確、配件附著點是否連續、前後遮擋是否合理、尾巴根部是否連接正確、側背未知細節、角色大小／基準線、單一角色、無文字／Logo／多餘道具，以及原始畫風。若背面出現正面專屬配件（例如胸前領巾結）或尾巴換到不合理的一側，該視角直接 `fail`，最多重試一次。

...
- 生成前在內部完成一份「視角轉換檢查表」：對每個配件與非對稱特徵記錄附著部位、角色自身左右、朝向、四個視角的預期可見程度（不可見／部分可見／完整可見）、遮擋關係與根部連接。這份檢查不必在純圖片模式交付，但必須影響 prompt 與 QA。
...
生成 prompt 順序：身份鎖定與參考圖用途 → 必須保留的 3–8 個錨點與 fixed rules → 配件附著位置、角色自身左右與朝向 → 該視角的預期可見程度及遮擋規則 → 中性站姿、畫布、比例、基準線與鏡頭 → visual language、配色、材質、光線 → hard avoids 與未知細節政策。對背面視圖要明確寫出「位於前側且被身體遮擋的正面專屬元素不可見」，並說明哪些結構因繞過身體或延伸到背面而可見；不能只寫「正面配件不可見」或「保留配件」。

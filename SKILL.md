...
- 先判斷每個配件的附著位置、朝向與遮擋關係：例如胸前的蝴蝶結、領巾結、徽章或口袋，轉到背面後通常應被身體遮住；只有確實繞過頸部或延伸到背面的布料／結構才可見。這些例子不是固定清單，任何配件都適用同一原則。
...
- 生成前在內部完成一份「視角轉換檢查」：附著點、角色自身左右、前／後可見性、遮擋關係、尾巴根部與身體連接。這份檢查不必在純圖片模式交付，但必須影響 prompt 與 QA。
...
生成 prompt 順序：身份鎖定與參考圖用途 → 必須保留的 3–8 個錨點與 fixed rules → 配件附著位置及角色自身左右 → 唯一視角與前後遮擋規則 → 中性站姿、畫布、比例、基準線與鏡頭 → visual language、配色、材質、光線 → hard avoids 與未知細節政策。對背面視圖要明確寫出「正面配件不可見」及哪些後側結構可見，不能只寫「保留配件」。
...
逐張檢查並記錄：身份、臉型／頭部比例、身體比例、比例鎖定表的相對關係、服裝、招牌配件、主色／輔色、四分之三角度的臉部與配件連續性、角色自身左右是否正確、配件附著點是否連續、前後遮擋是否合理、非對稱特徵是否依空間關係連續、側背未知細節、角色大小／基準線、單一角色、無文字／Logo／多餘道具，以及原始畫風。若任一視角出現不符合附著位置或遮擋關係的正面專屬元素、非對稱特徵錯誤換側，該視角直接 `fail`，最多重試一次。
